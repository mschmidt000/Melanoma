                              ...
                              magnitude = "sca.LRscore", show_complex = TRUE)
{
  ...
  liana_mod <- liana_agg %>% filter(source %in% source_groups) %>%
    filter(target %in% target_groups) %>% rename(magnitude = !!magnitude) %>%
    rename(specificity = !!specificity) %>% unite(entities,
                                                  col = "interaction", sep = " -> ") %>% unite(c("source",
                                                                                                 "target"), col = "source_target", remove = FALSE)
  cbPalette <- scales::hue_pal()(20)
  suppressWarnings(ggplot(liana_mod, aes(x = interaction, y = target,
                                         colour = magnitude, size = specificity, group = target)) +
                     geom_point() + scale_color_gradientn(colours = viridis::viridis(20)) +
                     scale_size_continuous(range = c(5, 9)) + facet_grid(source ~
                                                                           ., space = "free", scales = "free", switch = "y") + theme_bw(base_size = 20) +
                     theme(legend.text = element_text(size = 12), axis.text.y = element_text(colour = scales::hue_pal()(length(unique(liana_mod$target))),
                                                                                             face = "bold", size = 12), axis.text.x = element_text(size = 12,
                                                                                                                                                   angle = 90, vjust = 0.5), legend.title = element_text(size = 12),
                           panel.spacing = unit(0.1, "lines"), strip.background = element_rect(fill = NA),
                           strip.text = element_text(size = 5, colour = "gray6")) +
                     scale_y_discrete(position = "right") + labs(x = "Interactions (Ligand -> Receptor)",
                               ...
                               magnitude = "sca.LRscore", show_complex = TRUE)
{
  ...
  suppressWarnings(ggplot(liana_mod, aes(x = interaction, y = target,
                     ...
                     scale_size_continuous(range = c(5, 9)) + facet_grid(source ~
                                                                           ., space = "free", scales = "free", switch = "y") + theme_bw(base_size = 20) +
                     theme(legend.text = element_text(size = 16), axis.text.y = element_text(colour = "black",
                                                                                             face = "bold", size = 23), axis.text.x = element_text(size = 18,
                                                                                                                                                   angle = 90, vjust = 0.5), legend.title = element_text(size = 18),
                           panel.spacing = unit(0.1, "lines"), strip.background = element_rect(fill = NA),
...
plot_lymphs_vs_mels <- function(liana_obj, seurat_obj, dataset){
  ...
  if(nrow(liana_subset) > 0){
    my_liana_dotplot2(liana_subset, source_groups = melanocytes,
