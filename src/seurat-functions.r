...
#'
#' @param runs character vector of run ids
#' @param paths character vector of individual run paths
#' @param input_data_path character vector of general input data path
#' @return list of seurat objects, one object per run
create_seurat_object <- function(runs = runs, paths = paths, input_data_path = input_data_path){
  
  for (i in seq_along(runs)) {
    runs_list <- list()
    runs_list[[runs[i]]] <- list()
    runs_list[[runs[i]]] <- Seurat::ReadMtx(
    ...
    runs_list[[runs[i]]] <- Seurat::CreateSeuratObject(counts = as.matrix(runs_list[[runs[i]]]), min.cells = 1, min.features = 1)
    runs_list[[runs[i]]][["percent.mt"]] <- Seurat::PercentageFeatureSet(runs_list[[runs[i]]], pattern = "^MT-")
    runs_list[[runs[i]]][["percent.rps"]] <- Seurat::PercentageFeatureSet(runs_list[[runs[i]]], pattern = "^RPS")
...
#' @return NULL
do_qc <- function(figures_path = figures_path){
  
  filename <- here::here(figures_path, "01_quality-check.pdf")
  ...
  for (i in seq_along(runs_list)) {
    ...
    print(Seurat::VlnPlot(object = runs_list[[runs[i]]], features = c("nFeature_RNA", "nCount_RNA", "percent.mt", "percent.rps")), main = runs[i])
    print(Seurat::FeatureScatter(runs_list[[runs[i]]], feature1 = "nCount_RNA", feature2 = "percent.mt"))
    print(Seurat::FeatureScatter(runs_list[[runs[i]]], feature1 = "nFeature_RNA", feature2 = "percent.mt"))
    print(Seurat::FeatureScatter(runs_list[[runs[i]]], feature1 = "percent.rps", feature2 = "percent.mt"))
...
#'
#' @param runs_list list of seurat objects to filter and plot
#' @param figures_path character vector of output figures path
#' @return list of seurat objects
do_filtering_and_qc <- function(runs_list = runs_list, figures_path = figures_path){
...
#' @return list of seurat objects
save_seurat_objects <- function(output_data_path = output_data_path, runs_list = runs_list){
 
  for (i in seq_along(runs_list)) {
    
    seurat_obj <- runs_list[[i]]
    filename <- here(output_data_path, paste0(names(runs_list)[i], "_seurat-obj.RData"))
