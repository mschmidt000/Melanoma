...
features_profiles_and_maps <- function(features, filename){
  ...
  if(length(features)==0){ return("No input features found")} else {
    
    for(i in seq_along(features)){
      
      set.genes <- ifelse(is.list(features), names(features[[i]]), names(features[i]))
      if(length(intersect(set.genes, rownames(env$indata)))==0){ next }
      ...
      barplot(feature.data, beside=TRUE,  cex.main = 0.6,
