...
obj_mel_ds <- subset(obj_mel, downsample = 300) %>%
                ScaleData(verbose = FALSE, features = rownames(obj_mel_ds))
rm(obj_mel)
env <- opossom.new(list(dataset.name="2022-05-26-melanocytes",
                        dim.1stLvlSom="auto",
                        dim.2ndLvlSom="auto",
...
env$indata <- as.matrix(SetAssayData(obj_mel_ds, slot = "scale.data", assay = "integrated"))
